...
public partial class WeatherForecastEntityEditUIBroker
{
    ...
    public async ValueTask LoadAsync(WeatherForecastId id)
    {
        LastResult = await Result<WeatherForecastId>.Create(id)
            .MapToResultAsync( (id) => _isLoaded ? Task.FromResult(LoadedResult) : this.LoadEntityAsync(id));
    }

    public ValueTask ResetItemAsync()
    {
        LastResult = Result.Success()
            .MapToResult( () => _isLoaded ? Result.Success() : NotLoadedResult)
    ...
    public async ValueTask SaveItemAsync(bool refreshOnNew = true)
    {
        LastResult = await Result.Success()
            ...
            .MapToResultAsync( () => _isLoaded ? this.UpdateEntityAsync(refreshOnNew): Task.FromResult(NotLoadedResult))
    ...
    {
        LastResult = await Result.Success()
            .MapToResultAsync(() => _isLoaded ? this.DeleteEntityAsync() : Task.FromResult(NotLoadedResult));
    }
}
