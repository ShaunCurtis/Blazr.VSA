...
/// </summary>
public sealed class InvoiceRecordHandler : IRequestHandler<InvoiceRecordRequest, Bool<InvoiceEntity>>
{
    ...
    public async Task<Bool<InvoiceEntity>> HandleAsync(InvoiceRecordRequest request, CancellationToken cancellationToken)
    {
        ...
        if (invoiceResult.HasException)
            return Bool<InvoiceEntity>.Failure(invoiceResult.Exception!);
        ...
        var invoiceItemsResult = await dbContext
            ...
            .BindAsync(provider => provider.Items.Select(item => DvoInvoiceItem.Map(item)).ToBoolMonad());

        if (invoiceItemsResult.HasException)
            return Bool<InvoiceEntity>.Failure(invoiceItemsResult.Exception!);
